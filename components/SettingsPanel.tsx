...
export default function Settings() {
  ...
  return (
    <div className="h-full w-full max-w-2xl mx-auto p-4 gap-6 flex flex-col text-zinc-100">
      {/* Header */}
      <div className="bg-white/5 p-6 rounded-xl border border-white/10 flex flex-col items-center gap-2 shadow-lg">
        <h1 className="text-3xl font-black tracking-tighter text-zinc-100 uppercase">System Settings</h1>
        <p className="text-zinc-400 text-sm">Gestionare identitate operator și parametri terminal.</p>
      </div>

      {isLoading && <p className="text-zinc-300 animate-pulse text-center text-xs">Processing request...</p>}
      {error && <p className="text-red-500 bg-red-500/10 p-2 rounded border border-red-500/20 text-center text-xs">{error}</p>}

      {/* Profile Settings Group */}
      <div className="flex flex-col gap-4">
        {/* Operator Name Field */}
        <div className="p-4 rounded-lg border border-white/10 bg-white/5 flex items-center gap-4 transition-all focus-within:border-white/40">
          ...
          <div className="flex flex-col flex-1">
            ...
            <input
              type="text"
              className="text-sm text-zinc-200 bg-transparent border-none focus:outline-none font-mono"
              value={name}
          ...
          <button
            ...
            disabled={isLoading}
            className="px-4 py-2 bg-white/10 hover:bg-white/20 text-zinc-100 rounded border border-white/30 text-[10px] font-bold transition-all disabled:opacity-50"
          >
            UPDATE
          </button>
        </div>
